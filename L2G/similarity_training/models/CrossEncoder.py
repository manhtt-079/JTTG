...
from transformers import LongformerTokenizerFast, AutoModelForSequenceClassification, AutoTokenizer, AutoConfig, LongformerForSequenceClassification
import numpy as np
import logging
import os
...
class CrossEncoderSentence():
    ...
    def smart_batching_sentences_collate(self, batch):
        ...
        for e_idx, example in enumerate(batch):
            ...
            #         sample_struc_dict[e_idx].append(len(texts[1]) - 1)
            if len(context_sents) > 15:
                context_sents = context_sents[:15]
            for sent in context_sents:
                texts[0].append(query)
                texts[1].append(sent)
                sample_struc_dict[e_idx].append(len(texts[1]) - 1)
    ...
    def smart_batching_sentences_collate_text_only(self, batch):
        ...
        for j in range( CTS ):
            ...
            batch_texts.append(texts[0][a:b])
            batch_texts.append(texts[1][a:b])
            tokenized = self.tokenizer(*batch_texts, padding=True, truncation='longest_first', return_tensors="pt", max_length=self.max_length, pad_to_max_length=True)
            all_tokenized.append(tokenized)
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            for batch_features, labels, sample_struc_dict, len_texts_batch in tqdm(train_dataloader, desc="Iteration", smoothing=0.05, disable=not show_progress_bar):
                ...
                if use_amp:
                    ...
                    model_predictions = torch.zeros((len_texts_batch, 1), device=self._target_device)
                    arr_idx = 0
                    for f_idx, features in enumerate(batch_features):
                        ...
                        model_predictions[arr_idx:arr_idx+model_prediction.logits.shape[0]] = model_prediction.logits
                        arr_idx += model_prediction.logits.shape[0]                    
                    final_idx = 0
                    final_model_prediction = torch.zeros((len(labels), 1), device=self._target_device)
                    for sample_idx in range(len(labels)):
                        idx_relevant = torch.tensor(sample_struc_dict[sample_idx]).to(self._target_device)
                        sample_predict = torch.index_select(model_predictions, 0, idx_relevant)
                        sample_predict = torch.max(sample_predict)
                        sample_predict = torch.reshape(sample_predict, (1, 1))
                        final_model_prediction[final_idx:final_idx+sample_predict.shape[0]] = sample_predict
                        final_idx += sample_predict.shape[0]

                    logits = activation_fct(final_model_prediction)
                    # del model_predictions
                    # del final_model_prediction
    ...
    def predict(self, sentences: List[List[str]],
        ...
        with torch.no_grad():
            for batch_features, sample_struc_dict, len_texts_batch in iterator:
                batch_preds = []
                sample_preds = torch.zeros((len_texts_batch, 1), device=self._target_device)
                arr_idx = 0
                for f_idx, features in enumerate(batch_features):
                    for name in features:
                        features[name] = features[name].to(self._target_device)
                    ...
                    sample_preds[arr_idx:arr_idx+model_predictions.logits.shape[0]] = model_predictions.logits
                    arr_idx += model_predictions.logits.shape[0]

                final_model_prediction = None
                for sample_idx in range(len(list(sample_struc_dict.keys()))):
                    idx_relevant = torch.tensor(sample_struc_dict[sample_idx]).to(self._target_device)
                    sample_predict = torch.index_select(sample_preds, 0, idx_relevant)
                    sample_predict = torch.max(sample_predict)
                    sample_predict = torch.reshape(sample_predict, (1, 1))
                    if sample_idx == 0:
...
class CrossEncoder():
    def __init__(self, model_name:str, tokenizer_model_name: str = None, num_labels:int = None, max_length:int = None, device:str = None, tokenizer_args:Dict = {},
                 default_activation_function = None):
        """
        A CrossEncoder takes exactly two sentences / texts as input and either predicts
        a score or label for this sentence pair. It can for example predict the similarity of the sentence pair
    ...
    def smart_batching_collate(self, batch):
        ...
        for example in batch:
            for idx, text in enumerate(example.texts):
                ...
                #     texts[idx].append(text.strip())
                # else:
                #     # corpus_tokenized = sent_tokenize(text.strip())
                #     corpus_tokenized = text.strip().split('\n')
                #     tokenized_corpus = [doc.split(" ") for doc in corpus_tokenized]
                #     bm25 = BM25Okapi(tokenized_corpus)
                #     tokenized_question = example.texts[0].split(" ")
                #     doc_scores = bm25.get_scores(tokenized_question)
                #     # print(doc_scores)
                #     ind_top_k = torch.topk(torch.tensor(doc_scores), k=1)
                #     ind_top_k = ind_top_k[1].numpy()
                #     full_text_corpus = corpus_tokenized[ind_top_k[0]].strip()
                #     if ind_top_k[0] != len(corpus_tokenized) - 1:
                #         full_text_corpus += (' ' + corpus_tokenized[ind_top_k[0] + 1].strip())
                #     texts[idx].append(full_text_corpus)
                    # if int(example.label) == 1:
                    #     print(texts[0][-1])
                    #     print(texts[idx][-1])
                    #     print('part 1: ', corpus_tokenized[ind_top_k[0]])
                    #     if ind_top_k[0] != len(corpus_tokenized) - 1:
                    #         print('part 2: ', corpus_tokenized[ind_top_k[0] + 1])
                    #     print('-----------------------')
    ...
    def smart_batching_collate_text_only(self, batch):
        ...
        for example in batch:
            for idx, text in enumerate(example):
                ...
                #     tokenized_corpus = [doc.split(" ") for doc in corpus_tokenized]
                #     bm25 = BM25Okapi(tokenized_corpus)
                #     tokenized_question = example[0].split(" ")
                #     doc_scores = bm25.get_scores(tokenized_question)
                #     # print(doc_scores)
                #     ind_top_k = torch.topk(torch.tensor(doc_scores), k=1)
                #     ind_top_k = ind_top_k[1].numpy()
...
class CrossEncoderLongformer():
    def __init__(self, model_name:str, tokenizer_model_name: str = None, num_labels:int = None, max_length:int = None, device:str = None, tokenizer_args:Dict = {},
        ...
        self.tokenizer = LongformerTokenizerFast.from_pretrained(tokenizer_model_name, **tokenizer_args)
        self.max_length = max_length
